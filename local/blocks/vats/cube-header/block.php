<header class="header">
    <div class="site-container">
        <div class="header__inner">
            <a class="header__logo" href="/">
                <img class="header__logo-img" src="/local/js/vatc/build/assets/images/logo-red.svg" alt="Logo">
            </a>
            <nav class="nav header__nav">
                <ul class="nav__list">
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#tariffs">Тарифы</a>
                    </li>
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#advantages">Преимущества</a>
                    </li>
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#possibilities">Характеристики</a>
                    </li>
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#coverage">Покрытие</a>
                    </li>
                </ul>
            </nav>
            <button class="header__call button-2" href="#application">Позвонить</button>
            <button class="header__btn"><span></span><span></span><span></span>
            </button>
        </div>
    </div>
</header>

<div class="main-menu">
    <div class="main-menu__container">
        <div class="site-container">
            <nav class="nav">
                <ul class="nav__list">
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#tariffs">Тарифы</a>
                    </li>
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#advantages">Преимущества</a>
                    </li>
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#possibilities">Характеристики</a>
                    </li>
                    <li class="nav__list-item">
                        <a class="nav__list-link" href="#coverage">Покрытие</a>
                    </li>
                </ul>
            </nav>
            <button class="header__call button-2" href="#coverage">Позвонить</button>
        </div>
    </div>
</div>