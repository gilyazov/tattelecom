...
if ($bAllowAccess)
{
	...
	{
		$arSection["NAME"] = component . phpstr_repeat(" . ", $arSection["DEPTH_LEVEL"]) . $arSection["NAME"];
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		{
            $recaptchaResponse = GoogleReCaptcha::checkClientResponse($_REQUEST['recaptcha_response']);
            if ($recaptchaResponse['score'] < 0.5){
                $arResult["ERRORS"][] ='Не пройдена проверка на робота. Ваш результат: ' . $recaptchaResponse['score'];
            }
